...
extern void launchMultiplyMatrices(int* mat_A, int* mat_B, int* mat_C, int size, int myrank);
extern int* allocate_memory(int size);
extern void free_memory(int* ptr);
...
int main(int argc, char* argv[]) {
    ...
    if (myrank == 0) {
        printf("Running MPI/CUDA program on %d ranks | %d X %d processor grid\n", npes, q, q);
    ...
    int* local_A = allocate_memory(local_num_elements);
    int* local_B = allocate_memory(local_num_elements);
    int* local_C = allocate_memory(local_num_elements);
    int* temp_A = allocate_memory(local_num_elements);
    ...
    for (int i = 0; i < q; i++) {
        ...
        // matrix multiplication of the smaller chunks is done inside of a CUDA kernel
        launchMultiplyMatrices(temp_A, local_B, local_C, block_size, myrank);
    ...
    end_time = getticks();
    if (myrank == 0) {
        printf("Total time running MPI/CUDA is %lf seconds \n", (double)(end_time - start_time) / (double)512000000.0);
    }
